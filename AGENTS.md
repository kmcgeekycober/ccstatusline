...
> My main interest is tweaking the token usage display thresholds and default color scheme.
> The `renderer.ts` and `model-context.ts` files are the most relevant for those changes.
